 ...
 * Returns 0 on success, -1 on failure
 */
int	fd(t_ast_node *ast, t_fds *fd, t_redirection *redir, t_shell *shell)
{
	char	*expanded_filename;
	int		result;

	(void)ast;
	expanded_filename = expand_variables(redir->file_or_delimiter,
	...
	{
		fd->out_new = open(expanded_filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
		if (fd->out_new == -1)
			result = (perror("minishell"), -1);
	}
	else if (redir->type == REDIR_APPEND)
	{
		fd->out_new = open(expanded_filename, O_WRONLY | O_CREAT | O_APPEND, 0644);
		if (fd->out_new == -1)
			result = (perror("minishell"), -1);
	}
	else if (redir->type == REDIR_IN)
	{
		fd->in_new = open(expanded_filename, O_RDONLY, 0644);
		if (fd->in_new == -1)
			result = (perror("minishell"), -1);
	}
