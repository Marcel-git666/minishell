...
/*   Updated: 2025/07/30 14:55:09 by mmravec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expand_compound_token(char *compound, t_env *env, int exit_status)
{
	...
	printf("DEBUG: expand_compound_token input = '%s'\n", compound);
	...
	while (i < part_count)
	{
		...
		{
			// Single quoted - no expansion, keep literal content
			printf("DEBUG: single quoted part '%s' - no expansion\n", parts[i].content);
		...
		{
			// Regular text or double quoted content - can contain variables
			printf("DEBUG: processing text/double-quoted '%s'\n", parts[i].content);
	...
	printf("DEBUG: expand_compound_token result = '%s'\n", result);
	return (result);
}
